...
(function( $ ) {
    ...
    $.fn.drawr = function( action, param ) {
        ...
        plugin.activate_brush = function(brush){
			// {"icon":"mdi mdi-play-circle mdi-24px","name":"start","size":5,"alpha":0.8,"order":1}
        	var context = this.getContext("2d", { alpha: this.settings.enable_tranparency });
			...
			if(typeof brush.name!=="undefined" && brush.name == 'startAnnotation'){
				//openCanvasOnFullScreen();
				$('.drawr-toolbox-brush .drawr-tool-btn:not(.active)').removeClass('active').css({ 'background': 'rgb(238, 238, 238)', 'pointer-events': '', 'cursor': 'pointer' });
			}
        ...
        plugin.create_button = function(toolbox,type,data,css){
			/* console.log(`create_button called icon=${data.icon} ## ${data} ## ${JSON.stringify(data)}`); */
        	var self=this;
			let customClass = (data.hasOwnProperty('eClass')) ? data.eClass : '';
        	var el = $("<a class='drawr-tool-btn "+ data.name +" "+ customClass +"' title='"+ data.tooltip +"' style='cursor:pointer;float:left;display:block;margin:0px;'><i class='" + data.icon + "'></i></a>");
        ...
        plugin.initialize_canvas = function(width,height,reset){
			...
			context.fillRect(0,0,width,height);
			var parent_width = window.innerWidth;; //$(this).parent().innerWidth();
			var parent_height = window.innerHeight; //$(this).parent().innerHeight();
		...
		this.each(function() {
			...
			if ( action === "start") {
	            ...
	            $(".drawr-toolbox").hide();
	            $(".drawr-toolbox-brush").attr('id', 'canvasBrush').show();
			...
			//not an action, but an init call
				...
				//brush dialog
        		currentCanvas.$brushToolbox = plugin.create_toolbox.call(currentCanvas,"brush",{ left: $(currentCanvas).parent().offset().left, top: $(currentCanvas).parent().offset().top },"Annotate",80);
	    		...
	    		plugin.create_button.call(currentCanvas,currentCanvas.$brushToolbox[0],"toggle",{"icon":"mdi mdi-palette-outline mdi-24px", "name":"colorOption", "tooltip":"Choose Color"}).on("touchstart.drawr mousedown.drawr",function(){
	    			currentCanvas.$settingsToolbox.toggle();
					$('.drawr-toolbox-settings').css({'top': $('#canvasBrush').offset().top, 'left': $('#canvasBrush').offset().left + 100});
	    		});
	    		/*
				// Disabled by AMAN - No need for Now
	    		...
	    		currentCanvas.$undoButton=plugin.create_button.call(currentCanvas,currentCanvas.$brushToolbox[0],"action",{"icon":"mdi mdi-undo-variant mdi-24px", "name":"undoLastState", "tooltip":"Undo Last State"}).on("touchstart.drawr mousedown.drawr",function(){
...
/*
// Commented by Aman - This option is not needed for now
jQuery.fn.drawr.register({
...
//effectCallback
/*
// Commented by Aman - This option is not needed
jQuery.fn.drawr.register({
...
//effectCallback
jQuery.fn.drawr.register({
	icon: "mdi mdi-play-circle mdi-24px",
	...
	eClass: 'drag-disable',
	activate: function(brush,context){ console.log(`start button activate called`) },
	deactivate: function(brush,context){ console.log(`start button deactivate called`) },
	drawStart: function(brush,context,x,y,size,alpha,event){ console.log(`start button drawStart called`) },
	drawRotatedImage: function (context, image, x, y, angle, size) { console.log(`start button drawRotatedImage called`) },
	drawSpot: function(brush,context,x,y,size,alpha,event) { console.log(`start button drawSpot called`) },
	drawStop: function(brush,context,x,y,size,alpha,event){ console.log(`start button drawStop called`) }
});